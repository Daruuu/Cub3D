...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/05/28 09:34:31 by yamir             #+#    #+#             */
/*   Updated: 2025/06/01 00:30:22 by cpujades         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	collidex(t_vars *vars, t_player *player)
{
	...
	if (player->motion.x > 0)
		card = NORTH;
	pt = get_portal(vars, side, floor(player->pos.y), card);
	if (pt)
...
void	collidey(t_vars *vars, t_player *player)
{
	...
	if (player->motion.y > 0)
		card = EAST;
	pt = get_portal(vars, floor(player->pos.x), side, card);
	if (pt)
		translate_portal_3(&(player->pos), card, pt);
	side = floor(player->pos.y + player->motion.y);
